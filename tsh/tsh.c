...
void start()
{
   static void (*op_func[])() = {OpPut, OpGet, OpGet, OpExit, OpRshell} ;

   while (TRUE) {				/* read operation on TSH port */
...
void OpRshell()
{
   ...
   //printf("Incoming request 2 :: %s\n", in.buffer);

   in.proc_id = ntohl(in.proc_id);
   if (guardf(in.host, in.proc_id)) return;
   //printf("Incoming request 2 :: %s\n", in.buffer);
   //printf("Incoming command lenght :: %lu\n", sizeof(in.buffer));

   strncpy(out.buffer, in.buffer, sizeof(in.buffer));
   //out.buffer = in.buffer;
   //printf("after copy request 2 :: %s\n", out.buffer);

   buffer = sbot_exec(out.buffer);
   //printf("Output :: %s", buffer);

   strcpy(out.buffer, buffer);
   out.status = htons((short int)SUCCESS) ;
   writen(newsock, (char *)&out, sizeof(tsh_rshell_ot)) ;
